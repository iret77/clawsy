...
class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    /// Bootstraps the OpenClaw agent with Clawsy pairing knowledge.
    /// Called once after the setup code is decoded — uses only the HTTP API (no WS pairing needed).
    /// Writes clawsy-pair.sh to the workspace and notifies the agent about Clawsy.
    static func bootstrapAgentKnowledge(gatewayURL: String, token: String) {
        // Build HTTP base URL from the gateway WebSocket URL
        let httpBase: String
        ...
        // The pairing script content (same as tools/clawsy-pair.sh)
        let pairScript = #"""
#!/bin/bash
# clawsy-pair.sh — Auto-generated by Clawsy app on first connect
TIMEOUT=120; LINK_ONLY=false
while [[ $# -gt 0 ]]; do
  case "$1" in --link-only) LINK_ONLY=true; shift ;; --timeout) TIMEOUT="$2"; shift 2 ;; *) shift ;; esac
done
SETUP_CODE=$(openclaw qr --setup-code-only 2>/dev/null)
if [[ -z "$SETUP_CODE" ]]; then echo "ERROR=Gateway not running"; exit 1; fi
...
import json,sys
data=json.load(sys.stdin); pending=data.get('pending',[])
if pending: print(pending[-1].get('requestId',''))
" 2>/dev/null || true)
  if [[ -n "$PENDING" ]]; then
    for i in 1 2 3; do
      RESULT=$(openclaw devices approve "$PENDING" 2>&1)
      if echo "$RESULT" | grep -q "Approved\|approved"; then echo "APPROVED=${PENDING}"; exit 0; fi
      sleep 1
    done
  fi
  sleep 2
...
"""#
        ...
        let writeCommand = """
mkdir -p ~/.openclaw/workspace/tools && cat > ~/.openclaw/workspace/tools/clawsy-pair.sh << 'CLAWSY_PAIR_EOF'
\(pairScript)
CLAWSY_PAIR_EOF
chmod +x ~/.openclaw/workspace/tools/clawsy-pair.sh
"""

        func invoke(tool: String, args: [String: Any], completion: ((Bool) -> Void)? = nil) {
        ...
        // 1. Write clawsy-pair.sh via exec
        invoke(tool: "exec", args: ["command": writeCommand]) { ok in
            os_log("Clawsy bootstrap: write clawsy-pair.sh %{public}@",
                   log: OSLog(subsystem: "ai.clawsy", category: "Bootstrap"),
                   type: .info, ok ? "OK" : "FAILED")
        }

        // 2. Notify agent that Clawsy is now installed and teach "pair clawsy"
        let systemMsg = """
[Clawsy] macOS Companion App wurde soeben vom User verbunden. \
CLAWSY.md wurde in dein Workspace installiert — lies sie bei der nächsten Gelegenheit. \
Wenn jemand "pair clawsy" oder "clawsy verbinden" schreibt: \
führe `bash ~/.openclaw/workspace/tools/clawsy-pair.sh --link-only` aus, \
sende dem User den Link (clawsy://pair?code=...) und starte den Watcher im Hintergrund: \
`bash ~/.openclaw/workspace/tools/clawsy-pair.sh`. Ich approve das Pairing automatisch.
"""
        invoke(tool: "sessions_send", args: ["sessionKey": "main", "message": systemMsg])
    }
