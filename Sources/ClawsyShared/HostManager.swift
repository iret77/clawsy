...
public class HostManager: ObservableObject {
    ...
    //
    // These properties and methods bridge the old NetworkManager-based API
    // that ContentView and AppDelegate still depend on. They will be removed
    // once the UI migration to ConnectionState-based views is complete.

    /// Legacy NetworkManager instances (one per host), kept for ContentView callbacks
    public var networkManagers: [UUID: NetworkManager] = [:]

    /// The active NetworkManager (legacy — use activeConnection instead)
    public var activeNetworkManager: NetworkManager? {
        guard let id = activeHostId ?? profiles.first?.id else { return nil }
        return networkManagers[id]
    }

    /// Legacy connection status string (derived from ConnectionState)
    @Published public var connectionStatus: String = "STATUS_DISCONNECTED"

    /// Legacy connection attempt count
    ...
    /// Legacy clawsy-aware flag
    @Published public var isServerClawsyAware: Bool = false

    /// Legacy pairingRequestId as non-optional String
    public var legacyPairingRequestId: String {
        pairingRequestId ?? ""
    }

    /// Connect all hosts using legacy NetworkManager (transition period).
    /// Once the UI is migrated to the new engine, this is replaced by connectAll().
    public func connectAllLegacy(setupCallbacks: @escaping (NetworkManager, HostProfile) -> Void) {
        for profile in profiles {
        ...
        subscribeToActiveLegacyNM()
    }

    /// Connect a single host using legacy NetworkManager
    public func connectHostLegacy(_ id: UUID, setupCallbacks: @escaping (NetworkManager, HostProfile) -> Void) {
        guard let profile = profiles.first(where: { $0.id == id }) else { return }
        let nm: NetworkManager
    ...
    /// Subscribe to active legacy NetworkManager's @Published properties
    private func subscribeToActiveLegacyNM() {
        // Reuse activeCancellables — clear any existing subscriptions
        activeCancellables.removeAll()
        ...
        nm.$connectionAttemptCount.receive(on: DispatchQueue.main)
            .assign(to: \.connectionAttemptCount, on: self)
            .store(in: &activeCancellables)
        ...
        nm.$isServerClawsyAware.receive(on: DispatchQueue.main)
            .assign(to: \.isServerClawsyAware, on: self)
            .store(in: &activeCancellables)
        nm.$pairingRequestId.receive(on: DispatchQueue.main)
            .map { $0.isEmpty ? nil : $0 }
            .assign(to: \.pairingRequestId, on: self)
            .store(in: &activeCancellables)
    ...
    public func disconnectAllLegacy() {
