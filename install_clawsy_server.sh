...
- **Workflow & Automations:** When Clawsy sends a `file.sync_triggered` event, check the directory (especially `changedPath`) for a `.clawsy` file using `nodes.invoke` (`file.get`).
- **Rule Processing:** If a `.clawsy` file exists, it contains instructions for that folder. Compare the file list with `memory/clawsy_processed.json` to identify new files.
- **Loop Prevention:** Only process files NOT present in `memory/clawsy_processed.json`. After processing, add the filename and its last modified timestamp to the JSON file. NEVER process `.clawsy` or `.DS_Store` files.
EOF
fi
