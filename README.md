...
*   **Agent Automations (`.clawsy`):** Create a `.clawsy` file in any subfolder to define specific background rules (e.g., "Summarize all new PDFs"). The agent reads these rules and processes new files automatically.
*   **Loop Protection:** A smart state-tracking mechanism (`clawsy_processed.json`) ensures the agent never gets stuck in infinite processing loops.
*   **Non-Blocking Notifications:** Whenever the agent creates, modifies, or deletes a file in your workspace, Clawsy alerts you with a native, non-intrusive macOS notification.
